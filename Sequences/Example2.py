...
class Sequence(SequenceBase):
   ...
   def loop(self):
      for nCol in range(len(self.columns)):
         self.setRgb(self.columns[nCol], self.pattern[(self.nIndex + nCol) % len(self.pattern)])
      self.show()
      self.delay(self.delayMs)
      self.nIndex = (self.nIndex + 1) % len(self.pattern)